...
DATASET_PATH = '../data/diabetes_dataset.csv'  # CSV file with all patient data
...
N_ITER_TUNING = 20
CV_FOLDS      = 3 # 3-fold cross validation during tuning


SEARCH_N_JOBS = 1  # how many CPU cores to use for RandomizedSearchCV
...
LIFESTYLE_FEATURES = [  #   self-reported lifestyle and demographics
    'age',
    'bmi',
    'waist_to_hip_ratio',
...
def validate_no_duplicate_features(features, model_name='MODEL'):
    """
    Extra safety net: makes sure a feature list has no duplicate entries.
    This is exactly what would have caught the earlier 'glucose_fasting'
    bug immediately, with a clear message, instead of a confusing
    XGBoost crash deep in RandomizedSearchCV.
    """
    seen = set()
    dupes = set()
    for f in features:
...
def build_base_estimator(name, random_state):
    ...
    """
    # IMPORTANT: n_jobs=1 on every individual estimator here (not -1).
    # RandomizedSearchCV already parallelizes across CPU cores via its
    # own n_jobs setting (see SEARCH_N_JOBS below). If the estimators
    # ALSO spawn their own parallel workers, you get nested parallelism —
    # each outer worker process tries to launch its own inner worker
    # pool, multiplying memory usage (every one holds its own copy of
    # the training data + partially built trees) until Windows runs
    # out of RAM and the worker pool crashes with a MemoryError /
    # BrokenProcessPool. One level of parallelism (the search) is
    # enough and is dramatically lighter on memory.
    estimators = {
