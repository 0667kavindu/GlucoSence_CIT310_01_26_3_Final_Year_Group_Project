...
#   Original  → Logistic Regression, Random Forest, XGBoost
#   New       → K-Nearest Neighbors, Gradient Boosting, Extra Trees
# Every model goes through RandomizedSearchCV to find the best
# hyperparameters before we pick a winner and save it.

import os

# Limit BLAS threads per process — has to happen before numpy/sklearn
# are imported. Otherwise each parallel worker spins up its own thread
# pool on top of joblib's, and the CPU gets oversubscribed. On Windows
# this was actually crashing Logistic Regression's solver randomly.
...
DATASET_PATH = 'diabetes_dataset.csv'  # where the CSV lives
...
# Worker processes for RandomizedSearchCV. -1 (all cores) can overload
# weaker machines and cause memory crashes, so we scale to core count
# minus 1 instead. Drop this to 2 or 1 if you're still seeing crashes.
_cpu_count    = os.cpu_count() or 2
SEARCH_N_JOBS = max(1, _cpu_count - 1)
...
# (glucose_fasting is a lab value, so it stays out of this list —
# it belongs in BLOOD_AND_MEDICALLY_MEASURABLE_FEATURES above)
LIFESTYLE_FEATURES = [
    'age',
    'bmi',
    'waist_to_hip_ratio',
...
def validate_no_duplicate_features(features, model_name='MODEL'):
    """Catches accidental duplicate column names in a feature list early."""
    seen = set()
    dupes = set()
    for f in features:
        if f in seen:
        ...
        seen.add(f)
    if dupes:
        raise ValueError(f"\n❌ Duplicate feature(s) found for {model_name}: {sorted(dupes)}")
    print(f"✓ No duplicate features for {model_name}")


def encode_categorical_features(X):
    """
    Turns text columns into numbers. Gender gets one-hot encoded since
    it has no order; everything else gets label-encoded (0, 1, 2...).
    Returns the updated DataFrame plus the encoders (needed later for
    transforming new patient data the same way).
    """
    label_encoders = {}
...
def build_base_estimator(name, random_state):
    ...
    """
    # n_jobs=1 here on purpose — RandomizedSearchCV already parallelizes
    # across cores, so letting each model ALSO parallelize internally
    # just doubles up and eats memory (this was crashing Extra Trees).
    estimators = {
...
def tune_and_train(
    ...
    """
    Tunes and trains one model: tries n_iter random hyperparameter
    combos with cross-validation, refits the best one on the full
    training set, then scores it on the test set.

    Using RandomizedSearchCV instead of GridSearchCV because trying
    every combination on 100k rows would take way too long — random
    search gets close to the same result much faster.
    """
    ...
    search = RandomizedSearchCV(
        ...
        cv                  = skf,
        random_state        = random_state,
        n_jobs              = SEARCH_N_JOBS,     # capped, not -1 — see config comment above
        pre_dispatch        = '2*n_jobs',        # don't queue more jobs than workers can hold at once
...
def checkpoint_path(model_type, model_name):
    """Builds a file path for a given model's checkpoint."""
    safe_name = model_name.replace(' ', '_').replace('-', '_')
    return os.path.join(CHECKPOINT_DIR, f'{model_type}__{safe_name}.pkl')


def save_checkpoint(model_type, model_name, result):
    """Saves a finished model's result so we don't lose it if training crashes later."""
    os.makedirs(CHECKPOINT_DIR, exist_ok=True)
    path = checkpoint_path(model_type, model_name)
    with open(path, 'wb') as f:
        pickle.dump(result, f)
    print(f'     💾 Checkpoint saved: {path}')


def load_checkpoint(model_type, model_name):
    """Loads a model's result if we already trained it in a previous run."""
    path = checkpoint_path(model_type, model_name)
...
print(f'Detected CPU cores : {_cpu_count}')
print(f'Using SEARCH_N_JOBS: {SEARCH_N_JOBS}  (lower this in the config section above if you hit crashes/slowness)')
...
# Make sure neither feature list has accidental duplicates — this is
# the exact check that would have caught the 'glucose_fasting' bug
# immediately with a clear message, instead of a confusing XGBoost
# crash 250+ seconds into training.
validate_no_duplicate_features(FULL_MODEL_FEATURES,       'FULL MODEL')
validate_no_duplicate_features(SIMPLIFIED_MODEL_FEATURES, 'SIMPLIFIED MODEL')
...
for model_name, cfg in MODEL_CONFIG.items():
    ...
    # Skip models we already finished in a previous (crashed) run
    cached = load_checkpoint('full', model_name)
    if cached is not None:
        print(f'\n  ⏩ Skipping {model_name} — found existing checkpoint (CV F1: {cached["cv_f1"]:.4f})')
        results_full[model_name] = cached
        continue
    ...
    save_checkpoint('full', model_name, result)
    results_full[model_name] = result
...
for model_name, cfg in MODEL_CONFIG.items():
    ...
    # Skip models we already finished in a previous (crashed) run
    cached = load_checkpoint('simplified', model_name)
    if cached is not None:
        print(f'\n  ⏩ Skipping {model_name} — found existing checkpoint (CV F1: {cached["cv_f1"]:.4f})')
        results_simp[model_name] = cached
        continue
    ...
    save_checkpoint('simplified', model_name, result)
    results_simp[model_name] = result
